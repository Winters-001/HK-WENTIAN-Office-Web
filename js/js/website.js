...
(function() {
  ...
  if (advGrid) {
    var iconMap = { shield: "\u{1F6E1}", truck: "\u{1F69A}", search: "\u{1F50D}", layers: "\u{1F5CA}" };
    content.advantages.items.forEach(function(item) {
      var card = document.createElement("div");
      card.className = "advantage-card";
      card.innerHTML =
        '<div class="advantage-icon">' + (iconMap[item.icon] || "\u25C6") + '</div>' +
  ...
  if (contactInfo) {
    var contactIconMap = { phone: "\u{1F4DE}", mail: "\u2709", map: "\u{1F4CD}" };
    content.contact.items.forEach(function(item) {
      var div = document.createElement("div");
      div.className = "contact-info-item";
      div.innerHTML =
        '<div class="contact-info-icon">' + (contactIconMap[item.icon] || "\u25C6") + '</div>' +
